...
import { AdjustmentsHorizontalIcon, ArrowSmallUpIcon, ArrowSmallDownIcon, ChevronUpDownIcon, PaintBrushIcon, ArchiveBoxIcon } from './icons';
...
const ColumnResizer = ({ onResize, onResizeEnd }) => {
    ...
    return (<div ref={ref} className="absolute top-0 right-0 h-full w-2 cursor-col-resize opacity-0 hover:opacity-100 bg-sky-500/50"></div>);
};
const DraggableHeader = ({ column, columnIndex, sortConfig, onSort, checksForPreview, onResize, onResizeEnd, theme, width, }) => {
    const ref = useRef(null);
    const [dragState, setDragState] = useState({ closestEdge: null });
    useEffect(() => {
        const el = ref.current;
        if (!el)
            return;
        return combine(draggable({
            ...
            onGenerateDragPreview: ({ nativeSetDragImage }) => {
                setCustomNativeDragPreview({
                    ...
                    render: ({ container }) => {
                        ...
                        root.render(<div className="bg-white rounded-md shadow-lg border border-slate-300">
                                    <table className="min-w-full">
                                        <thead className="bg-slate-100">
                                            <tr><th className="px-4 py-2 text-sm font-medium text-slate-700">{column.label}</th></tr>
                                        </thead>
                                        <tbody>
                                            {checksForPreview.map(check => (<tr key={check.id}><td className="px-4 py-1 text-sm text-slate-600 border-t truncate">{formatCell(check, column.key)}</td></tr>))}
                                        </tbody>
                                    </table>
                                </div>);
            ...
            onDragEnter: (args) => setDragState({ closestEdge: args.self.data.closestEdge }),
            onDrag: (args) => setDragState({ closestEdge: args.self.data.closestEdge }),
    ...
    return (<th ref={ref} scope="col" data-key={column.key} className={`px-6 py-3 text-left text-xs font-medium uppercase tracking-wider select-none transition-colors relative group ${headerTextColorClass}`} style={{ width: width ? `${width}px` : 'auto' }}>
...
const ArchiveView = ({ checks, onSelectCheck, onBack, searchTerm, visibleColumns, onVisibleColumnsChange, columnWidths: persistedWidths, onColumnWidthsChange, archiveTheme, themes, onOpenThemePicker }) => {
    const [sortConfig, setSortConfig] = useState(null);
    const [isColumnDropdownOpen, setIsColumnDropdownOpen] = useState(false);
    const [lastDroppedColumnIndex, setLastDroppedColumnIndex] = useState(null);
    ...
    useEffect(() => {
        return monitorForElements({
            onDrop({ source, location }) {
                ...
                const closestEdgeOfTarget = destination.data.closestEdge;
    ...
    useEffect(() => {
        if (lastDroppedColumnIndex === null)
            return;
        const table = tableRef.current;
        if (!table)
            return;
        const th = table.querySelector(`thead th:nth-of-type(${lastDroppedColumnIndex + 1})`);
        if (th) {
    ...
    const handleResize = useCallback((key, delta) => {
        if (!resizingColumn || resizingColumn.key !== key) {
            const headerElement = tableRef.current?.querySelector(`th[data-key="${key}"]`);
    ...
    const columns = useMemo(() => visibleColumns
        .map(key => ALL_CHECK_FIELDS.find(f => f.key === key))
        .filter((c) => Boolean(c)), [visibleColumns]);
    ...
    return (<main className="container mx-auto p-4 sm:p-6 lg:p-8">
                ...
                <div className="flex justify-between items-center mb-4 border-b pb-4">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div className="relative" ref={columnDropdownRef}>
                            ...
                            {isColumnDropdownOpen && (<div className="absolute right-0 mt-2 w-64 bg-white border rounded-md shadow-lg z-10 max-h-80 overflow-y-auto">
                                    <p className="px-3 py-2 text-xs font-semibold text-slate-500 bg-slate-50 border-b">Visible Columns</p>
                                    <div className="p-2">
                                        {ALL_CHECK_FIELDS.map(field => (<label key={field.key} className="flex items-center w-full px-2 py-1.5 text-sm text-slate-700 rounded-md hover:bg-slate-50">
                                                <input type="checkbox" checked={visibleColumns.includes(field.key)} onChange={() => handleToggleColumn(field.key)} className="h-4 w-4 rounded border-gray-300 text-sky-600 focus:ring-sky-500"/>
                ...
                {checks.length > 0 ? (<div className="overflow-x-auto">
                        <table ref={tableRef} className="min-w-full divide-y divide-slate-200 border-separate border-spacing-0" style={{ tableLayout: 'fixed' }}>
                            <thead className={headerClasses}>
                                <tr>
                                    {columns.map((col, index) => (<DraggableHeader key={col.key} column={col} columnIndex={index} sortConfig={sortConfig} onSort={handleSort} checksForPreview={filteredChecks.slice(0, 5)} onResize={(delta) => handleResize(col.key, delta)} onResizeEnd={handleResizeEnd} theme={theme} width={columnWidths[col.key]}/>))}
                                </tr>
                            </thead>
                            <tbody className="bg-white divide-y divide-slate-200">
                                {filteredChecks.map(check => (<tr key={check.id} onClick={() => onSelectCheck(check)} className={`${rowHoverClass} cursor-pointer`}>
                                        {columns.map(col => (<td key={col.key} className="px-6 py-4 whitespace-nowrap text-sm text-slate-700 truncate">
                                                {formatCell(check, col.key)}
                                            </td>))}
                                    </tr>))}
                            </tbody>
                        </table>
                    </div>) : (<div className="text-center py-16 px-4 border-2 border-dashed border-slate-200 rounded-lg">
