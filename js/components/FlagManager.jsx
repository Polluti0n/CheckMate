...
const FlagManager = ({ isOpen, flags, onClose, onUpdateFlags }) => {
    const [editingFlag, setEditingFlag] = useState(null);
    const handleSave = () => {
        if (!editingFlag || !editingFlag.name || !editingFlag.color)
            return;
        let updatedFlags;
        if (editingFlag.id) {
            updatedFlags = flags.map(f => f.id === editingFlag.id ? editingFlag : f);
        }
        else {
            updatedFlags = [...flags, { ...editingFlag, id: `flag-${uuidv4()}` }];
        }
    ...
    if (!isOpen)
        return null;
    return (<div className="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity z-50">
                    ...
                    <div className="p-6">
                        ...
                        <div className="mt-4 max-h-60 overflow-y-auto pr-2">
                            <ul className="space-y-2">
                                {flags.map(flag => (<li key={flag.id} className="flex items-center justify-between p-2 bg-slate-50 rounded-md">
                        ...
                        <div className="mt-6 border-t pt-4">
                            <h4 className="text-md font-medium text-gray-800">{editingFlag?.id ? 'Edit Flag' : 'Add New Flag'}</h4>
                             <div className="mt-2 space-y-3">
                                <input type="text" placeholder="Flag name" value={editingFlag?.name || ''} onChange={e => setEditingFlag(prev => ({ ...prev, name: e.target.value }))} className="w-full bg-slate-50 border border-slate-300 text-slate-900 rounded-md shadow-sm py-2 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-sky-300 focus:border-sky-500"/>
                                <div className="flex flex-wrap gap-2">
                                    {colors.map(color => (<button key={color.bg} onClick={() => setEditingFlag(prev => ({ ...prev, color: color.bg, textColor: color.text }))} className={`w-8 h-8 rounded-full ${color.bg} ${editingFlag?.color === color.bg ? 'ring-2 ring-offset-2 ring-sky-500' : ''}`}/>))}
                                </div>
