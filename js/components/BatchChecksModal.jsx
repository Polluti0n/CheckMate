...
const BatchChecksModal = ({ isOpen, batch, checks, onClose, onSelectCheck }) => {
    if (!isOpen || !batch)
        return null;
    ...
    return (<div className="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity z-50" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div className="flex items-center justify-center min-h-screen p-4 text-center">
                <div className="relative bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-2xl sm:w-full">
                    <div className="p-6">
                        ...
                        <div className="mt-4">
                            {checksInBatch.length > 0 ? (<div className="overflow-y-auto max-h-[60vh]">
                                    <table className="min-w-full divide-y divide-slate-200">
                                        ...
                                        <tbody className="bg-white divide-y divide-slate-200">
                                            {checksInBatch.map(check => (<tr key={check.id} onClick={() => handleCheckClick(check)} className="hover:bg-slate-50 cursor-pointer">
                                ...
                                </div>) : (<p className="text-slate-500 text-center py-8">No checks found for this batch.</p>)}
                        </div>
                         <div className="mt-6 border-t pt-4 flex justify-end">
                            <button type="button" onClick={onClose} className="inline-flex justify-center rounded-md border border-slate-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-slate-700 hover:bg-slate-50">
                                Close
                            </button>
                        </div>
                    </div>
