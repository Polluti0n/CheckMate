...
const ThemeSwatch = ({ theme }) => {
    ...
    return (<div className="w-16 h-16 rounded-full border-2 border-slate-700 overflow-hidden shadow-inner group-hover:scale-110 transition-transform duration-200" title={theme.name}>
...
const ThemePickerModal = ({ isOpen, onClose, themes, onSetTheme, target }) => {
    if (!isOpen || !target)
        return null;
                ...
                <div className="relative bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all my-8 sm:max-w-md sm:w-full">
                    <div className="p-6">
                        ...
                        <div className="mt-8 flex flex-wrap justify-center gap-x-6 gap-y-8">
                            {themes.map(theme => (<button key={theme.id} onClick={() => handleThemeSelect(theme.id)} className="flex flex-col items-center gap-3 group focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-500 rounded-lg p-1 w-24">
