...
const BatchHistoryView = ({ batches, checks, onSelectCheck, onBack }) => {
    ...
    return (<>
                    ...
                    {batches.length > 0 ? (<div className="overflow-x-auto">
                            <table className="min-w-full divide-y divide-slate-200">
                                ...
                                <tbody className="bg-white divide-y divide-slate-200">
                                    {batches.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).map(batch => (<tr key={batch.id} onClick={() => setSelectedBatch(batch)} className="hover:bg-slate-50 cursor-pointer">
            ...
            <BatchChecksModal isOpen={!!selectedBatch} batch={selectedBatch} checks={checks} onClose={() => setSelectedBatch(null)} onSelectCheck={onSelectCheck}/>
        </>);
};
