...
const DraggableField = ({ field, isDragging, sourceZone }) => {
    ...
    return (<div ref={ref} className={`px-3 py-2 border rounded-md bg-white hover:bg-slate-50 cursor-grab ${isDragging ? 'opacity-40' : ''}`}>
...
const PreferencesModal = ({ isOpen, onClose, currentPreferences, onSave }) => {
        ...
        return monitorForElements({
            onDragStart: ({ source }) => {
                if (source.data.type === 'field') {
                    setDragState({ type: 'dragging', fieldKey: source.data.fieldKey, sourceZone: source.data.sourceZone });
                }
            },
    ...
    const handleDropOnCardZone = (targetZone) => {
        ...
        setPrefs(prev => {
            ...
            if (sourceZone) {
                ...
                if (keyOfFieldInTarget)
                    delete newLayout[keyOfFieldInTarget];
                newLayout[targetZone] = fieldKey;
            }
    ...
    const DropZone = ({ fields, onDrop, title, className }) => {
        const ref = useRef(null);
    ...
    const CardDropZone = ({ zone, onDrop }) => {
        const ref = useRef(null);
        const [isDraggedOver, setIsDraggedOver] = useState(false);
        ...
        const sampleText = { title: "Payor Name", topRight: "$123.45", subtitle: "Category Name", body1: "Longer text for memo that might wrap...", body2: "Another line of body text", footerLeft: "Check #1234", footerRight: "10/26/2023" };
        return (<div ref={ref} className={`p-1 border border-dashed rounded-md transition-colors min-h-[2.5rem] flex items-center ${isDraggedOver ? 'border-sky-500 bg-sky-50' : 'border-slate-300'}`}>
