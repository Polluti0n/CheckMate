...
import { Check, Flag, CardLayoutZone, CheckField, UserPreferences, CheckCategory } from '../types';
...
import { communityIcon, CalendarDaysIcon, FlagIcon as FlagFilledIcon, HashtagIcon, UsDollarIcon, FlagIcon, EyeIcon, EyeSlashIcon } from './icons';
...
const renderZoneContent = (
    ...
    if (ZoneComponent) {
        return (
            <ZoneComponent 
                ...
                formattedValue={fieldData.key === 'flags' ? renderFlags(check, allFlags, false) : fieldData.value} 
    ...
    if (fieldData.key === 'flags') {
        return renderFlags(check, allFlags, false);
    }
...
export const ClassicCard: React.FC<CardProps> = ({ check, allFlags, cardLayout, ZoneComponent, preferences }) => {
    ...
    const borderColor = categoryInfo ? (preferences.darkMode ? categoryInfo.color.dark.border : categoryInfo.color.border) : (preferences.darkMode ? 'border-slate-600' : 'border-slate-500');
        ...
        <div className={`bg-white dark:bg-gray-800 p-2 rounded-md shadow-sm border border-l-4 hover:shadow-lg cursor-pointer active:cursor-grabbing transition-all duration-200 flex flex-col justify-between ${borderColor} hover:scale-105 min-h-[140px]`}>
            <div className="flex flex-col flex-grow space-y-1">
                <div className="flex justify-between items-start gap-2">
                    {/* Title */}
                    {renderZoneContent(check, allFlags, cardLayout, 'title', "font-semibold text-slate-800 dark:text-white text-base truncate", ZoneComponent, "flex-grow min-w-0")}
                    {/* TopRight */}
                    {renderZoneContent(check, allFlags, cardLayout, 'topRight', "font-bold text-slate-900 dark:text-white text-base", ZoneComponent, "flex-shrink-0")}
                </div>
                {/* Subtitle */}
                {renderZoneContent(check, allFlags, cardLayout, 'subtitle', "text-xs text-slate-500 dark:text-gray-400 font-medium truncate", ZoneComponent)}
                {/* Body1 */}
                {renderZoneContent(check, allFlags, cardLayout, 'body1', "text-sm text-slate-700 dark:text-gray-300 break-words pt-1", ZoneComponent)}
            </div>
            <div className="flex justify-between items-end mt-1 pt-1 border-t border-slate-100 dark:border-gray-700 gap-2">
                {/* FooterLeft */}
                {renderZoneContent(check, allFlags, cardLayout, 'footerLeft', "text-xs text-slate-500 dark:text-gray-400 truncate", ZoneComponent, "flex-grow min-w-0")}
                
                {/* FooterRight */}
                {renderZoneContent(check, allFlags, cardLayout, 'footerRight', "text-xs text-slate-500 dark:text-gray-400 font-mono ml-2", ZoneComponent, "flex-shrink-0")}
            </div>
        </div>
    );
};
            ...
            <div className="p-3 flex flex-col gap-2">
                <div className="flex justify-between items-baseline border-b-2 border-dashed border-stone-300 dark:border-gray-600 pb-2 mb-1 gap-2">
                    <div className="flex flex-col flex-grow min-w-0 gap-1">
                        ...
                        {renderZoneContent(check, allFlags, cardLayout, 'title', "font-bold text-stone-800 dark:text-white text-sm truncate", ZoneComponent)}
                    </div>
                    {/* Top Right (Amount) */}
                    {renderZoneContent(check, allFlags, cardLayout, 'topRight', "text-lg font-bold text-stone-900 dark:text-white tabular-nums tracking-tight", ZoneComponent, "flex-shrink-0")}
                </div>
                ...
                <div className="space-y-1.5 text-xs text-stone-600 dark:text-gray-300">
                    {activeZones.map((zone, idx) => {
                        ...
                        if (ZoneComponent) {
                            const content = fieldData.key === 'flags' ? renderFlags(check, allFlags, preferences.darkMode) : fieldData.value;
                            return (
                ...
                <div className="mt-2 pt-2 border-t-2 border-dashed border-stone-300 dark:border-gray-600 flex justify-between items-center">
                    <div className="scale-90 origin-left opacity-80 flex items-center">
                         {renderFlags(check, allFlags, preferences.darkMode)}
                    </div>
                    {/* Footer Right area usually used for Verified/Date in ledger. We map it above, but this slot is decorative */}
                    <div className="text-[10px] text-stone-400 dark:text-gray-500 italic">
                        TRANSACTION RECORD
                    </div>
                </div>
            </div>
...
export const ModernCard: React.FC<CardProps> = ({ check, allFlags, cardLayout, ZoneComponent, preferences }) => {    
    const categoryInfo = CHECK_TYPE_COLORS[check.category];
    ...
    const finalColor = preferences.darkMode ? colorSet.dark.border.replace('border','bg') : colorSet.border.replace('border','bg');

    const Icon = categoryInfo ? categoryConfig[check.category].icon : communityIcon;
    ...
    return (
        <div className="relative mt-6 hover:scale-[1.02] transition-transform duration-200 cursor-pointer group min-h-[200px]">
            <div className="bg-white dark:bg-gray-800 shadow-xl relative rounded-lg overflow-hidden flex flex-col h-full border border-slate-100 dark:border-gray-700">
                
                {/* Header */}
                <div className={`p-4 relative flex justify-between items-center ${finalColor}`}>
                    ...
                    <div>
                        {renderZoneContent(check, allFlags, cardLayout, 'topRight', "uppercase tracking-widest text-xs text-white bg-black/30 backdrop-blur-sm py-1 px-2 rounded shadow-lg font-bold", ZoneComponent)}
                    </div>
                </div>

                {/* Body */}
                <div className="p-4 text-slate-700 dark:text-gray-300 flex flex-col flex-grow justify-between gap-4">
                    <div className="w-full space-y-2">
                        {renderZoneContent(check, allFlags, cardLayout, 'title', "text-sm font-bold text-slate-900 dark:text-white leading-tight", ZoneComponent)}
                        ...
                        {(ZoneComponent || subtitle.value) && (
                            <div className="flex items-center gap-1">
                                ...
                                {renderZoneContent(check, allFlags, cardLayout, 'subtitle', "text-xs uppercase font-bold tracking-wide text-slate-500 dark:text-gray-400", ZoneComponent)}
                            </div>
                        )}
                        ...
                        {(ZoneComponent || body1.value) && (
                            <div className="flex items-center gap-1">
                                ...
                                {renderZoneContent(check, allFlags, cardLayout, 'body1', "text-xs text-slate-600 dark:text-gray-300 leading-snug line-clamp-3", ZoneComponent)}
                            </div>
                        )}
                    </div>
                    ...
                    {hasFlags && (
                        <div className="flex flex-wrap gap-2 pt-2">
                            {sortedFlags.map(flag => {
                                const styles = getChipStyles(flag.color || '', preferences.darkMode);
                                return (
                                    <span 
                                        key={flag.id}
                ...
                {/* Footer */}
                <div className="flex justify-between items-center px-4 py-3 border-t border-slate-200 dark:border-gray-700 bg-slate-50 dark:bg-gray-700 text-slate-600 dark:text-gray-300 gap-2">
                    {(ZoneComponent || footerLeft.value) && (
                        <div className="flex items-center gap-1">
                            ...
                            {renderZoneContent(check, allFlags, cardLayout, 'footerLeft', "text-xs font-bold text-slate-900 dark:text-white flex items-center", ZoneComponent)}
                        </div>
                    )}
                    {(ZoneComponent || footerRight.value) && (
                        <div className="flex items-center gap-1">
                            ...
                            {renderZoneContent(check, allFlags, cardLayout, 'footerRight', "text-xs font-bold text-slate-900 dark:text-white flex items-center", ZoneComponent)}
                        </div>
                    )}
                </div>
            </div>
...
export const CheckStyleCard: React.FC<CardProps> = ({ check, allFlags, cardLayout, ZoneComponent, preferences }) => {
    ...
    const bgColor = getColor(preferences.darkMode ? categoryInfo?.dark.bg : categoryInfo.bg);

    const renderContent = (zone: CardLayoutZone, classes: string) => {
        return renderZoneContent(check, allFlags, cardLayout, zone, classes, ZoneComponent);
    }
    ...
    return (
        <div className={`bg-white dark:bg-gray-800 rounded-lg p-2 shadow-sm border border-slate-200 dark:border-gray-700`}>
            <div className={`check-aspect-ratio relative w-full transition-all duration-200 ${backgroundClass}`} style={bgColor as React.CSSProperties}>
                <div className={`check-aspect-ratio relative w-full shadow-inner flex flex-col p-2 ${preferences.darkMode ? 'text-white' : 'text-black'} transition-all duration-200 border ${themeBorderColor} font-${fontTheme}-printed`}>
                    <div className={`p-2 ${preferences.darkMode ? 'text-white' : 'text-black'} border-2 ${themeBorderColor} h-full flex flex-col`}>
                        {/* Static content from old component */}
                        ...
                        <div className="flex-grow flex flex-col justify-around my-1">
                            ...
                            {options.showAmountInWords && <div className="flex items-center">
                        ...
                        <div className="flex justify-between items-end flex-shrink-0">
                            ...
                            <div className="w-1/3 flex items-end">
                                {options.showSignature && <p className={`flex-grow border-b border-dotted border-gray-400 text-[4cqw] text-center truncate pb-0 leading-tight ${`font-${fontTheme}-signature`}`}>
                                    {check.payor}
                                </p>}
                            </div>
                        </div>
