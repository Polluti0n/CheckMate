...
const ProcessBatchModal: React.FC<ProcessBatchModalProps> = ({ isOpen, checks, onClose, onProcessBatch }) => {
        ...
        try {
            ...
            const sheetDefs = {
                'Tracking Log': {
                    ...
                    dataMap: (c: Check) => {
                        let type = 'Misc';
                        if (c.category === CheckCategory.HOMEOWNER_LOCKBOX || c.category === CheckCategory.MISC_HOMEOWNER_INCOME) type = 'Homeowner';
                        else if (c.category === CheckCategory.MISC_NON_HOMEOWNER_INCOME) type = c.department || 'GL';
                        else if (c.category === CheckCategory.COMMUNITY_ARCHIVES) type = 'Settlement';
                ...
                'HO LockBox': {
                    ...
                    dataMap: (c: Check) => [new Date(c.date), c.associationName, c.checkNumber, c.amount, c.payor, c.accountNumber, c.memo]
                },
                'Misc Income Homeowner': {
                    ...
                    dataMap: (c: Check) => [new Date(c.date), c.associationName, c.payor, c.accountNumber, c.amount, c.checkNumber, c.chargeType, c.memo]
                },
                'Misc Income Non Homeowner': {
                    ...
                    dataMap: (c: Check) => [c.associationName, c.checkNumber, c.amount, c.glCode, c.glDescription, c.depositingBank, c.department, c.memo]
                },
                'Attorney': {
                    headers: ['Date', 'Association', 'Check #', 'Amount', 'Payor', 'Account #', 'Memo'],
                    columns: [{width: 12, numFmt: 'm/d/yy'}, {width: 25}, {width: 15}, {width: 15, numFmt: '"$"#,##0.00'}, {width: 25}, {width: 20}, {width: 40}],
